...
```hcl
module "lambda-cron" {
    source = "github.com/pbs/terraform-lambda-cron-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/lambda-cron'
gh -R 'pbs/terraform-lambda-cron-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/lambda-cron'
rm -f release.tar.gz
```
...
```bash
git remote add -f lambda-cron git@github.com:pbs/terraform-lambda-cron-module.git
git subtree add --prefix terraform/modules/lambda-cron lambda-cron main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/lambda-cron lambda-cron $REF --squash
```
